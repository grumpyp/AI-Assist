...
class QueueingTheoryModel(db.Model):
    ...
    call_analysis = db.relationship("CallAnalysis", lazy=True, viewonly=True)
...
class QueueingTheoryBestMatch(db.Model):
    ...
    matched_employees = db.relationship("Employee", secondary=employee_queueing_theory_best_match, lazy=True, viewonly=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow)
