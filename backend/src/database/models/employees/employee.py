...
employee_call = db.Table('employee_call',
                         db.Column('call_id', db.String(36), db.ForeignKey('call.id'), nullable=True),
                         db.Column('employee_id', db.String(36), db.ForeignKey('employee.id')))

employee_team = db.Table('employee_team',
                         db.Column('team_id', db.String(36), db.ForeignKey('team.id'), nullable=True),
                         db.Column('employee_id', db.String(36), db.ForeignKey('employee.id')))
...
employee_queueing_theory_best_match = db.Table('employee_queueing_theory_best_match',
                                               db.Column('queueing_theory_best_match:id', db.String(36),
                                                         db.ForeignKey('queueing_theory_best_match.id'), nullable=True),
                                               db.Column('employee_id', db.String(36), db.ForeignKey('employee.id')))
...
class Employee(db.Model):
    ...
    calls = db.relationship("Call", secondary=employee_call, viewonly=True)
    teams = db.relationship("Team", secondary=employee_team, viewonly=True)
    queueing_theory_best_match = db.relationship("QueueingTheoryBestMatch",
                                                 secondary=employee_queueing_theory_best_match, viewonly=True)
